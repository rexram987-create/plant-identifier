...
def main():
    ...
    # Quantize only the transformer/classifier matrix multiplications.
    # Leaving the patch-embedding Conv in FP32 avoids ConvInteger browser/runtime
    # compatibility problems, and dynamic weight quantization does not require
    # synthetic calibration data that can badly distort activation ranges.
    print("Quantizing transformer MatMul/Gemm weights to INT8...")
    if INT8.exists():
        INT8.unlink()
    quantize_dynamic(
    ...
    # Regression test: the quantized graph must load and run and expose one
    # logit per OpenPlants label.
    session = ort.InferenceSession(str(INT8), providers=["CPUExecutionProvider"])
    input_name = session.get_inputs()[0].name
    sample = np.random.default_rng(7).uniform(-1.0, 1.0, size=(1, 3, 224, 224)).astype(np.float32)
    ...
    # Sanity-check that the classifier is not numerically flattened.
    logits = outputs[0][0]
    spread = float(np.max(logits) - np.min(logits))
    if not np.isfinite(logits).all() or spread < 1e-3:
        raise SystemExit(f"Quantized logits look invalid/flat (spread={spread})")

    print(f"Dynamic INT8 inference OK: input={input_name}, output_shape={outputs[0].shape}, logit_spread={spread:.4f}")
